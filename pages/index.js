...
export default function Home() {
  ...
  async function loadNFTs() {
    ...
    const marketplaceContract = new ethers.Contract(nftMarketAddress, NFTMarketplace.abi, provider)
  ...
  async function buyNFT(nft) {
    ...
    const transaction = await contract.createNFTItem(nftAddress, nft.tokenId, {
      value: price
    })
  ...
  if (loadingState === true && !nfts.length) return (
    <h1 className='px-20 py-10 text-3xl'>No items right now</h1>
  )
